...
puts 'Welcome to Ebook Generator development box'.yellow
...
['console'].each do |project|
  if File.directory?("/vagrant/projects/#{project}")
    puts "project #{project} already cloned"
  else
    puts "cloning #{project}"
    Git.clone("git@github.com:Knowlodge/ebook-generator-#{project}.git", "/vagrant/projects/#{project}")
  end
...
puts 'you are now ready to use the development box from knowlodge!'
puts 'press enter to continue...'
gets
